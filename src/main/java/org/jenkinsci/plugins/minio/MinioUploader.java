...
public final class MinioUploader extends Recorder implements SimpleBuildStep {
    ...
    try {
      ...
      for (String startPath : expanded.split(",")) {
        for (FilePath path : ws.list(startPath, exclude)) {
          ...
          final int workspacePath = FileHelper.getSearchPathLength(ws.getRemote(), startPath.trim());
  ...
  /**
   * Descriptor for {@link MinioUploader}. Used as a singleton. The class is marked as public so that it can be accessed
   * from views.
   *
   * <p>
